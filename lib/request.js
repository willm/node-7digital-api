...
function prepare(data, consumerkey, options) {
	...
	var formatters = {
		'application/x-www-form-urlencoded': function(data) { return data; },
		'application/json': JSON.stringify
	};

	if (!options.is2Legged) {
	...
	return formatters[options.contentType](data);
}
...
function dispatchSecure(path, httpMethod, requestData, headers, authtype,
	...
	if (httpMethod === 'DELETE') {
		return oauthClient.delete(url, token, secret, cbWithDataAndResponse);
	}

	if ( oauthMethod ) {
		logger.info('DATA: ' + requestData);
		return oauthMethod(url, token, secret, requestData,
			mergedHeaders['Content-Type'], cbWithDataAndResponse);
	}
...
function dispatch(url, httpMethod, data, headers, hostInfo, credentials,
	...
	// TODO: Move this to the schema as it will no longer work now 
	// that we're serializing the data
